...
class Contextual_Attention_Module(nn.Module):
    ...
    def forward(self, f, b, mask: Optional[torch.Tensor] = None, ksize: int = 3, stride: int = 1,
                fuse_k: int = 3, softmax_scale: float = 10., training: bool = True,
                fuse: bool = True):
        ...
        raw_w = raw_w.contiguous().view(raw_int_bs[0], raw_int_bs[2] // self.rate,
        ...
        raw_w_groups = torch.split(raw_w, 1,
                                   dim=0)  # Split tensors by batch dimension; tuple is returned
        y = []
        offsets = []
        k = fuse_k
        scale = softmax_scale
        ...
        for xi, wi, raw_wi in zip(f_groups, w_groups, raw_w_groups):
            ...
            wi : separated patch tensor along batch dimension of back; (B=1, O=32*32, I=128, 
            KH=3, KW=3)
            raw_wi : separated tensor along batch dimension of back; (B=1, I=32*32, O=128, KH=4, 
            KW=4)
            '''
            # conv for compare
            wi = wi[0]
            escape_NaN = torch.tensor([1e-4], requires_grad=True, dtype=torch.double).cuda()
            ...
            if fuse:
                ...
                yi = yi.permute(0, 3, 1,
                                2)  # make all of depth to spatial resolution, (B=1, I=1,
                # H=32*32, W=32*32)
    ...
    def extract_patches(self, x, kernel: int = 3, stride: int = 1):
        x = self.padding(x)
...
# to check the reason
def down_sample(x, scale_factor: Optional[float] = None, mode: str = 'nearest',
...
#     if x.is_cuda:
#         if device:
#             grid = torch.tensor(grid).cuda(device)
#         else:
...
def to_var(x, volatile=False, device=None):
    if torch.cuda.is_available():
        if device:
            ...
            x = x.cuda()
    return torch.tensor(x, requires_grad=True, volatile=volatile)
